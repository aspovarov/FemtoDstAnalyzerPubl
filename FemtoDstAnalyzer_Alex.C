...
//_________________
void FemtoDstAnalyzer_Alex(const Char_t *inFile = "st_physics_12150008_raw_4030001.femtoDst.root",
  ...
  if( strncmp(energy, "62GeV",5) == 0) {
    ...
    Double_t sys_DCA=1.5;
    nSigm = 3.0;
    badRuns ={11080057,11080060,11080061,11080062,11080069,
  ...
  const Char_t *systematic[] = {"SysVtxZ", "SysnHits", "SysDCA"};
  const Char_t *systematic_text[] = {"45", "18", "1.5"};

  const Char_t *path="/mnt/pool/rhic/2/demanov97/hpc_scripts";
  //const Char_t *path="/mnt/pool/1/aspovarov/basov/test";
  ...
  TH1D *hNumberEventsPID[3][2];
	...
	//***************PROFILES FOR Q-VECTOR***************//
  TProfile2D *tp_fill_Qx1[2][3], *tp_fill_Qy1[2][3];
	TProfile2D *tp_fill_Qx2[3][n], *tp_fill_Qx3[3][n], *tp_fill_Qy2[3][n], *tp_fill_Qy3[3][n];

  TProfile2D *tp_read_Qx1[2][3], *tp_read_Qy1[2][3];
  TProfile2D *tp_read_Qx2[3][n], *tp_read_Qx3[3][n], *tp_read_Qy2[3][n], *tp_read_Qy3[3][n];
  
  //***************PROFILES FOR COS AND SIN***************//
  ...
  TProfile2D *tp_meanPt_PID_sys[n][2][3][3];
    ...
    for(Int_t l = 0; l < 3; l++) {
      ...
      for(Int_t i = 0; i < n; i++) {
        ...
        tp_fill_Qy2[l][i] = new TProfile2D(Form("tp_Qy2%s%s",direction[l],ngap[i]),
        Form("<Q_{y}> for #psi_{2} %s %s;RunID;cent",direction[l],gap[i]),runIdBins ,runIdRange[0],runIdRange[1],9,0,9);

        tp_fill_Qx3[l][i] = new TProfile2D(Form("tp_Qx3%s%s",direction[l],ngap[i]),
    ...
    f1 = new TFile(Form("%s/OUT/NoRe_%s.root", path,energy),"READ");
    for(Int_t dir = 0; dir < 3; dir++) {
      for(Int_t i = 0; i < n; i++) {
        tp_read_Qx2[dir][i] = (TProfile2D*) f1 -> Get( Form("tp_Qx2%s%s",direction[dir],ngap[i]) );
        tp_read_Qx3[dir][i] = (TProfile2D*) f1 -> Get( Form("tp_Qx3%s%s",direction[dir],ngap[i]) );
        tp_read_Qy2[dir][i] = (TProfile2D*) f1 -> Get( Form("tp_Qy2%s%s",direction[dir],ngap[i]) );
        tp_read_Qy3[dir][i] = (TProfile2D*) f1 -> Get( Form("tp_Qy3%s%s",direction[dir],ngap[i]) );
      }
      for(Int_t det = 0; det < 2; det++) {
        tp_read_Qx1[det][dir] = (TProfile2D*) f1 -> Get( Form("tp_Qx1%s%s",detector[det+1],direction[dir]) );
        tp_read_Qy1[det][dir] = (TProfile2D*) f1 -> Get( Form("tp_Qy1%s%s",detector[det+1],direction[dir]) );
      }
    }
    ...
    for(Int_t l = 0; l < 3; l++) {
	  ...
	  for(Int_t i = 0; i < n; i++) {
	    ...
	    tp_SqRes2[i] = new TProfile(Form("tp_SqRes2%sTPC",ngap[i]),
	    														Form("Resolution^{2} for v_{2} %s",gap[i]),9,0,9);
	    tp_SqRes3[i] = new TProfile(Form("tp_SqRes3%sTPC",ngap[i]),
	  ...
	  for(Int_t i = 0; i < n; i++) {
	  	...
	  	tp_meanPt_hadrons[i] = new TProfile2D(Form("tp_meanPtHadrons%s",ngap[i]),
	    Form("Mean p_{t} for bins v_{2} %s; bin; p_{t} [GeV/c]",gap[i]),30,0.2,3.2,9,0,9);

	    for(Int_t nsys = 0; nsys < 3; nsys++) {
	    	tp_meanPt_hadrons_sys[i][nsys] = new TProfile2D(Form("tp_meanPtHadrons%s%s",ngap[i],systematic[nsys]),
	    ...
	    for(Int_t mark = 0; mark < 2; mark++) {
	    	for(Int_t part = 0; part < 3; part++) {
	    		...
	    		Form("tp_meanPt%s%s%s",particles[part],sign[mark],ngap[i]),
	    		...
	    		for(Int_t nsys = 0; nsys < 3; nsys++) {
	    			...
	    			Form("tp_meanPt%s%s%s%s",particles[part],sign[mark],ngap[i],systematic[nsys]),
	  	...
	  	for(Int_t dir = 0; dir < 2; dir++) {
				...
				tp_v2hadrTPC[i][dir] = new TProfile2D(Form("tp_v2%sTPCHadrons%s",resol[dir],ngap[i]),
	                        Form("v_{2} of p_{t} and cent %s %s by TPC;p_{t} [GeV/c];cent",gap[i],resol[dir]),30,0.2,3.2,9,0,9);
	      tp_v3hadrTPC[i][dir] = new TProfile2D(Form("tp_v3%sTPCHadrons%s",resol[dir],ngap[i]),
	      ...
	      for(Int_t nsys = 0; nsys < 3; nsys++) {
	        ...
	        Form("tp_v2%sTPCHadronsSys%s%s",resol[dir],ngap[i],systematic[nsys]),
	        ...
	        Form("tp_v3%sTPCHadronsSys%s%s",resol[dir],ngap[i],systematic[nsys]),
	      ...
	      for(Int_t mark = 0; mark < 2; mark++) {
	      	for(Int_t part = 0; part < 3; part++) {
	      		...
	      		Form("tp_v2%sTPC%s%s%s",resol[dir],particles[part],sign[mark],ngap[i]), 
	          Form("v_{2} for %s of p_{t} and cent %s by TPC;p_{t} [GeV/c];cent",partLateX[p],gap[i]),
	          			30,0.2,3.2,9,0,9);

	          tp_v3pidTPC[i][dir][mark][part] = new TProfile2D(
	          Form("tp_v3%sTPC%s%s%s",resol[dir],particles[part],sign[mark],ngap[i]), 
	          ...
	          for(Int_t nsys = 0; nsys < 3; nsys++) {
	            ...
	            Form("tp_v2%sTPC%s%s%s%s",resol[dir],particles[part],sign[mark],ngap[i],systematic[nsys]), 
	            ...
	            Form("tp_v3%sTPC%s%s%s%s",resol[dir],particles[part],sign[mark],ngap[i],systematic[nsys]), 
    ...
    f2 = new TFile(Form("%s/OUT/Re_%s.root", path,energy),"READ");
    for(Int_t l = 0; l < 3; l++) {
      for(Int_t i = 0; i < n; i++) {
        for(Int_t j = 0; j < 4; j++) {
          tp_sinPsi2[l][i][j] = (TProfile2D*) f2 -> Get( Form("tp_%isinPsi2%s%s",j+1,direction[l],ngap[i]) );
          tp_cosPsi2[l][i][j] = (TProfile2D*) f2 -> Get( Form("tp_%icosPsi2%s%s",j+1,direction[l],ngap[i]) );
          tp_sinPsi3[l][i][j] = (TProfile2D*) f2 -> Get( Form("tp_%isinPsi3%s%s",j+1,direction[l],ngap[i]) );
          tp_cosPsi3[l][i][j] = (TProfile2D*) f2 -> Get( Form("tp_%icosPsi3%s%s",j+1,direction[l],ngap[i]) );
        }
      }
      for(Int_t det = 0; det < 2; det++) {
        for(Int_t j = 0; j < 20; j++) { 
          tp_sinPsi1[det][l][j] = (TProfile2D*) f2 -> Get( Form("tp_%isinPsi1%s%s",j+1,detector[det+1],direction[l]) );
          tp_cosPsi1[det][l][j] = (TProfile2D*) f2 -> Get( Form("tp_%icosPsi1%s%s",j+1,detector[det+1],direction[l]) );
        }
      }
    }
    ...
    /*////////////////////////////////////////////////////////////////////////////////////////*/
      ...
      hNumberEvents->Fill(event->runId());
        ...
        if ( femtoTrack->isTofTrack() ) {
          ...
          for(Int_t part = 0; part < 3; part++) {
            if( TMath::Abs( nSigma[part] ) < nSigm && SqM > SqMdown[part] && SqM < SqMup[part]) {
              hNumberEventsPID[part][q_charge]->Fill(event->runId());
            }
          }
        } //if( isTofTrack() )
    ...
    /*////////////////////////////////////////////////////////////////////////////////////////*/
      ...
      Recentering(Q1vec,Q2vecTPC,Q3vecTPC,RunID,cent,tp_read_Qx1,tp_read_Qy1,tp_read_Qx2,tp_read_Qy2,tp_read_Qx3,tp_read_Qy3);  
      ...
      for(Int_t dir = 0; dir < 3; dir++) {
          ...
          if( Q1vec[det][dir].Mod() != 0. ) {
            for(Int_t j = 0; j < 20; j++) { 
            	...
            	sinPsi1 = tp_sinPsi1[det][dir][j]->GetBinContent( tp_sinPsi1[det][dir][j]->FindBin(RunID, cent) );
              cosPsi1 = tp_cosPsi1[det][dir][j]->GetBinContent( tp_cosPsi1[det][dir][j]->FindBin(RunID, cent) );
        ...
        for(Int_t i = 0; i < n; i++) {
          if( Q2vecTPC[dir][i].Mod() != 0. && Q3vecTPC[dir][i].Mod() != 0.) {
            for(Int_t k = 0; k < 4; k++) {
              ...
              sinPsi2 = tp_sinPsi2[dir][i][k]->GetBinContent( tp_sinPsi2[dir][i][k] -> FindBin( RunID, cent ) );
              cosPsi2 = tp_cosPsi2[dir][i][k]->GetBinContent( tp_cosPsi2[dir][i][k] -> FindBin( RunID, cent ) );
              sinPsi3 = tp_sinPsi3[dir][i][k]->GetBinContent( tp_sinPsi3[dir][i][k] -> FindBin( RunID, cent ) );
              cosPsi3 = tp_cosPsi3[dir][i][k]->GetBinContent( tp_cosPsi3[dir][i][k] -> FindBin( RunID, cent ) );
